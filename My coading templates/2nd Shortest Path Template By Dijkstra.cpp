/// 2nd Shortest Path Template By Dijkstra
#include<bits/stdc++.h>
...
void Dijkstra(ll s)
{
    ...
    while(!PQ.empty())
    {
        Node u = PQ.top();
        PQ.pop();
        if(u.cost > dist2[u.node])
            continue;
        for(ll i = 0; i < graph[u.node].size(); i++)///for(Edge e: graph[u.node])
        {
            Edge e = graph[u.node][i];
            ...
            else if(dist2[e.v] > u.cost + e.w && dist[e.v] < u.cost + e.w)
            {
