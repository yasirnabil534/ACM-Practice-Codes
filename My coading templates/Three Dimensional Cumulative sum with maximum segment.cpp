...
void Solve(){
    int n1,n2,n3;
    cin>>n1>>n2>>n3;
    ll ar[n1+5][n2+5][n3+5];
    ll cum[n1+5][n2+5][n3+5];
    ll xam[n1+5][n2+5][n3+5];
    ms(ar, 0LL); ms(cum, 0LL); ms(xam, 0LL);
    for(int i = 1; i <= n1; i++){
        for(int j = 1; j <= n2; j++){
            for(int k = 1; k <= n3; k++){
                    cin>>ar[i][j][k];
    ...
    for(int i = 1; i <= n1; i++){
        for(int j = 1; j <= n2; j++){
            for(int k = 1; k <= n3; k++){
                ...
                cum[i][j][k] += cum[i][j-1][k];
                cum[i][j][k] += cum[i-1][j][k];
                cum[i][j][k] -= cum[i][j-1][k-1];
                cum[i][j][k] -= cum[i-1][j-1][k];
                cum[i][j][k] -= cum[i-1][j][k-1];
                cum[i][j][k] += cum[i-1][j-1][k-1];
            }
        }
    }
    ll p = INT_MIN;
        ...
        for(int j = 1; j <= n2; j++){
            for(int k = 1; k <= n3; k++){
                for(int x = 0; x < i; x++){
                    for(int y = 0; y < j; y++){
                        for(int z = 0; z < k; z++){
                            p = max(p, cum[i][j][k] - cum[i][j][z] - cum[i][y][k] - cum[x][j][k] + cum[i][y][z] + cum[x][y][k] + cum[x][j][z] - cum[x][y][z]);
                        }
                    }
                }
            }
