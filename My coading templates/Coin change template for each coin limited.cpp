...
int notes[55][2];
int n,k;         /// Here n is number of coins, k is the tergated notes
ll dp[55][1003];
ll change(int pos, int taka)
{
    if(pos>=n)
    {
        if(pos==n&&taka==0) return 1;
        return 0;
    }
    if(taka==0)
    ...
    if(taka<0)
        return 0;
    if(dp[pos][taka]!=-1) return dp[pos][taka]%100000007;
    ll p1 = 0;
    for(int i = 0; i <= notes[pos][1]; i++)
    {
        p1 = (p1 + change(pos + 1, taka-(notes[pos][0]*i)))%100000007;
    }
    return dp[pos][taka] = p1;
}
...
int main()
{
    ...
    scin(n);
    scin(k);
    for(int i = 0; i < n; i++) /// Getting coins input
    {
        scin(notes[i][0]);
    }
    for(int i = 0; i < n; i++) /// Getting number of ith coin
    {
        scin(notes[i][1]);
    }
    ll ans = change(0, k);
