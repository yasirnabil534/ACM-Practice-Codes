...
int main()
{
    ...
    {
        cnt = 0;
        num = 1;              /// Eta onno karone, amar b^p drkr chilo
        while(x%Prime[i]==0)
        {
            cnt++;
            x /= Prime[i];
            num *= Prime[i];  /// Eta lagbe na.
        }
        if(cnt>0)
        {
            Num.pb(num);      /// Eta lagbe na
