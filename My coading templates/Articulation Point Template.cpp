...
void DFS(ll u, ll p = -1)
{
    ...
    for(ll i = 0; i < graph[u].size(); i++)
    {
        if(visited[graph[u][i]]==0)
        {
            ...
            if(parent[u]==-1&&child>1) mark[u] = 1;
            if(parent[u]!=-1&&low[graph[u][i]]>=started[u]) mark[u] = 1;
        }
...
int main()
{
    ...
    while(1)
    {
        ...
        for(ll i = 1; i <= n; i++) if(mark[i]==1) sum++; /// mark[i] == 1 means i is an articulation point.
        pf("%lld\n",sum);
