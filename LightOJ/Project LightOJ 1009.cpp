...
void DFS(ll u)
{
    ...
    for(ll i = 0; i < graph[u].size(); i++)
        if(visited[graph[u][i]]==0) DFS(graph[u][i]);
}
...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        for(it = st.begin(); it != st.end(); it++)
            if(visited[*it]==0){BFS(*it,xx++); }
        ll cnt1[xx], cnt2[xx], xam = 0;
        ms(cnt1,0);
        ms(cnt2,0);
        for(it = st.begin(); it != st.end(); it++) if(visited[*it]>0){if(dist[*it]%2==0) cnt1[visited[*it]]++; else cnt2[visited[*it]]++;}
        for(ll i = 1; i < xx ; i++) xam += max(cnt1[i],cnt2[i]);
        CASE(t);
        pf("%lld\n",xam);
        for(it = st.begin(); it != st.end(); it++) graph[*it].clear();
        ms(visited,0);
        ms(dist,0);
    }
    return 0;
