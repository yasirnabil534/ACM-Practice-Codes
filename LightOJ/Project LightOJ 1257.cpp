...
void DFS(ll u, ll cst, ll key)
{
    ...
    for(ll i = 0; i < graph[u].size(); i++)
    {
        if(visited[graph[u][i]]==0)
            DFS(graph[u][i], cst + cost[u][i], key);
    }
}
...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        DFS(0,0,0);
        ll xam = -1, pos;
        for( ll i = 0; i < n; i++) if(iscost[0][i]>xam){xam = iscost[0][i]; pos = i;}
        ms(visited,0);
        DFS(pos,0,1);
        xam = -1;
        for( ll i = 0; i < n; i++) if(iscost[1][i]>xam){xam = iscost[1][i]; pos = i;}
        ms(visited,0);
        DFS(pos,0,2);
        CASEL(t);
        for(ll i = 0; i < n; i++)
        {
            pf("%lld\n",max(iscost[2][i], iscost[1][i]));
        }
