...
{
    graph[0][u].pb(Edge(v,w));
    graph[1][v].pb(Edge(u,w));
}

void Dijkstra(ll s, ll a)
{
    for(ll i = 0; i <=n; i++)
        dist[a][i] = 100000000000;
    dist[a][s] = 0;
    PQ.push(Node(s,0));
    while(!PQ.empty())
    {
        Node u = PQ.top();
        PQ.pop();
        if(u.cost!= dist[a][u.node])
            continue;
        for(ll i = 0; i < graph[a][u.node].size(); i++)///for(Edge e: graph[u.node])
        {
            Edge e = graph[a][u.node][i];
            if(dist[a][e.v] > u.cost + e.w)
            {
                dist[a][e.v] = u.cost + e.w;
                PQ.push(Node(e.v, dist[a][e.v]));
            }
        }
    }
}
...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        CASE(t);
        ll xam = -1;
        //for(ll i = 1; i <= n; i++) cout<<dist[0][i]<<' '<<dist[1][i]<<endl;
        for(ll i = 0; i < m; i++)
        {
            ll u,v,w;
            u = dist[0][ar[i].x];
        ...
        pf("%lld\n",xam);
        for(ll i = 1; i <= n; i++) for(ll j = 0; j < 2; j++) graph[j][i].clear();
    }
    return 0;
}
