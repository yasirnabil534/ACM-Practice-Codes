...
pdd NEW(pdd x, pdd y, double t){
    pdd ans;
    ans.ff = x.ff + (y.ff - x.ff)*t;
    ans.ss = x.ss + (y.ss - x.ss)*t;
    return ans;
}
...
void Solve(int tt){
    ...
    for(int i = 1; i < 100; i++){
        mid1 = lo + (hi - lo)/3;
        mid2 = lo + 2*(hi-lo)/3;
        double d1 = process(NEW(a,b,mid1),NEW(c,d,mid1));
        double d2 = process(NEW(a,b,mid2),NEW(c,d,mid2));
        if(d1 > d2){
            ans = min(ans, d2);
            lo = mid1;
        }
