...
struct Trio
{
    ll R, G, B;
    Trio():R(0),G(0),B(0){}
    Trio(ll a, ll b, ll c):R(a),G(b),B(c){}
};
ll n;
Trio ar[25];
ll dp[4][25];

ll func(int last, ll pos)
{
    if(pos==n)
        return 0;
    ll p1 = 100000000000, p2 = 100000000000, p3=100000000000;
    if(dp[last][pos]!=-1) return dp[last][pos];
    if(last==1)
    {
        p1 = ar[pos].R + func(2, pos + 1);
        p2 = ar[pos].R + func(3, pos + 1);
    }
    else if(last==2)
    {
        p1 = ar[pos].G + func(1, pos + 1);
        p2 = ar[pos].G + func(3, pos + 1);
    }
    else if(last==3)
    {
        p1 = ar[pos].B + func(1, pos + 1);
        p2 = ar[pos].B + func(2, pos + 1);
    }
    else if(last==0)
    {
        p1 = func(1,pos);
    ...
    return dp[last][pos] = min(p1,min(p2,p3));
}
