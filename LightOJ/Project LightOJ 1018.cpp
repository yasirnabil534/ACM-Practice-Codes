...
int Unset(int n, int pos) {return n=n&~(1<<pos);}
...
int func(int mask)
{
    ...
    int x, y;
    int nim = 100000000;
    for(x = 0; check(mask, x) && x < n; x++) ;
    for(y = 0; y < n; y++)
    {
        if(check(mask, y)==0 && x != y)
        {
            nim = min(1 + func(mask|Mask[x][y]), nim);
        }
    }
    return dp[mask] = nim;
}

bool colinear(pii x, pii y, pii z)
{
    int ans = (x.ff*(y.ss-z.ss)) - (x.ss*(y.ff-z.ff)) + (y.ff*z.ss - y.ss*z.ff);
    return ans == 0;
}
