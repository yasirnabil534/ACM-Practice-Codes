...
double func(int r, int b, int p){
    ...
    if(dp[r][b][p] != -1.0) return dp[r][b][p];
    double x = 0.0, y = 0.0;
    if(p==0){
        if(r>=0) x = ((r*1.0)/(r+b))*func(r-1, b, 1);
        if(b>=0) y = ((b*1.0)/(r+b))*func(r, b-1, 1);
    }
    else{
        if(b>=0) x = func(r, b - 1, 0);
    }
    return dp[r][b][p] = (x+y);
}
