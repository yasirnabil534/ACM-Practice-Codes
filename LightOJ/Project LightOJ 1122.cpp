...
ll func(int pos, int cnt){
    ...
    ll ret = 0;
    ret += func(pos, cnt + 1);
    if(pos < s && ar[pos+1]-ar[pos] <= 2) ret += func(pos+1, cnt+1);
    if(pos < s-1 && ar[pos+2]-ar[pos] <= 2) ret += func(pos+2, cnt+1);
    if(pos > 1 && ar[pos]-ar[pos-1]<=2) ret += func(pos-1, cnt+1);
    if(pos > 2 && ar[pos]-ar[pos-2]<=2) ret += func(pos-2, cnt+1);
