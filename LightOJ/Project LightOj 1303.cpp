...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        ll cnt = 0, ans = 0;
        ll running = 1, person;
        while((cnt< m*n)||(cnt>=m*n&&(ll)Q.size()<n))
        {
            running = ((running - 1) % m) + 1;
            while(!Q.empty()&&man[Q.front()][running]>0)
            {
                temp.push(Q.front());
                Q.pop_front();
            }
            ...
            if(seat[running]>0) Q.push_back(seat[running]);
            seat[running] = person;
            running++;
            ans++;
            //cout<<cnt<<' '<<(ll)Q.size()<<endl;
        }
        CASE(t);
        pf("%lld\n",ans*5);
    }
