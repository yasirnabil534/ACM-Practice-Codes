...
pll func(int pos, int chk, int now){
    if(pos == n){
        return mp(0,0);
    }
    if(dp[pos][chk][now] != mp(-1LL,-1LL)) return dp[pos][chk][now];
    pll p1 = {0,0}, p2 = {0,0}, ret = {0,0};
    if(chk==0){
        p1 = func(pos + 1, 0, 0);
        p2 = func(pos + 1, 0, 1);
    }
    ...
    if(now == 0)
        ...
        ret.ss += (p1.ss + p2.ss + p2.ff);
    }
    return dp[pos][chk][now] = ret;
}
