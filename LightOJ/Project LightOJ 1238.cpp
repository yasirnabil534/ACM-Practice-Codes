...
void BFS(ll u) // ky 0 for source, 1 for destination
{
    queue <ll> Q;
    Q.push(u);
    visited[u] = 1;
    ...
    RUN_CASE(t,T)
    {
        ...
        for(ll i = 0; i < n - 1; i++)
        {
            for(ll j = 0; j < m - 1; j++)
            {
                ...
                if(str[i][j]=='a') a = ar[i][j];
                if(str[i][j]=='b') b = ar[i][j];
                if(str[i][j]=='c') c = ar[i][j];
                if(str[i][j]=='h') h = ar[i][j];
                if(str[i+1][j]=='a') a = ar[i+1][j];
                if(str[i+1][j]=='b') b = ar[i+1][j];
                if(str[i+1][j]=='c') c = ar[i+1][j];
                if(str[i+1][j]=='h') h = ar[i+1][j];
                if(str[i][j+1]=='a') a = ar[i][j+1];
                if(str[i][j+1]=='b') b = ar[i][j+1];
                if(str[i][j+1]=='c') c = ar[i][j+1];
                if(str[i][j+1]=='h') h = ar[i][j+1];
            }
        }
        ms(visited,0);
        BFS(h);
        CASE(t);
        pf("%lld\n",max(dist[a],max(dist[b],dist[c])));
        for( ll i = 0; i < n*m; i++ ) graph[i].clear();
    }
    return 0;
}
