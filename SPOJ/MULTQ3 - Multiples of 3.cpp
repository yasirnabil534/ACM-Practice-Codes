...
void update(int node, int b, int e, int i, int j)
{
    ...
    Data p1 = tree[left];
    Data p2 = tree[right];
    for(int t = 0; t < tree[node].prop; t++){ p1.cycle();}
    for(int t = 0; t < tree[node].prop; t++){ p2.cycle();}
    tree[node] = p1 + p2;
}

Data query(int node, int b, int e, int i, int j, int carry = 0)
{
    if(i > e || j < b)
        return Zap;
    if(b >= i && e <= j){
        Data x = tree[node];
        for(int ii = 0; ii < (carry)%3; ii++)
            x.cycle();
        return x;
    }
    int left = ( node << 1 );
    int right = left + 1;
    int mid = (b + e)/2;
    Data p1 = query(left, b, mid, i, j, (carry%3 + tree[node].prop%3)%3);
    Data p2 = query(right, mid + 1, e, i, j, (carry%3 + tree[node].prop%3)%3);
    return p1 + p2;
}
