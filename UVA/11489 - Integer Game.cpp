...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        for(int i = 0; i < n; i++)
        {
            if(str[i]=='1'||str[i]=='4'||str[i]=='7') cnt[1]++;
            else if(str[i]=='2'||str[i]=='5'||str[i]=='8') cnt[2]++;
            else if(str[i]=='3'||str[i]=='6'||str[i]=='9') cnt[0]++;
        }
        cnt[0] %= 2;
        int tmp = (cnt[2] * 2 + cnt[1])%3;
        int state = 2;
        if(tmp != 0 && cnt[tmp]>0) state = reversestate(state);
        if(cnt[0]==1) state = reversestate(state);
        CASE(t);
        if(state==1) pf("S\n");
        else pf("T\n");
    }
