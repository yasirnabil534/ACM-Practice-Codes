...
ll func(ll pos, ll banaite_hobe)
{
    if(banaite_hobe==0)
        return 1;
    if(banaite_hobe<0||pos==21)
        return 0;
    if(dp[pos][banaite_hobe]>=0) return dp[pos][banaite_hobe];
    ll p1 = func(pos, banaite_hobe - coins[pos]);
    ll p2 = func(pos + 1, banaite_hobe);
    return dp[pos][banaite_hobe] = p1 + p2;
}

int main()
{
    for(ll i = 0; i <= 20; i++)
        coins[i] = POW(i+1ll,3ll);
    ll n;
    while(scln(n)!=EOF)
    {
        ms(dp,-1);
