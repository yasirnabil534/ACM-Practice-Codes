...
ll ar[50],br[50],cr[50];
ll w, n, x, cnt;
ll dp[35][1005];
ll dir[35][1005];
vector< pair <ll , ll> > vc;

ll func(ll pos, ll baki)
{
    if(baki==0)
        return 0;
    if(baki<0) return -100000000000;
    if(pos==n)
        return 0;
    if(dp[pos][baki]!=-1) return dp[pos][baki];
    ll p1 = 0,p2 = 0;
    if(baki-ar[pos]>=0) p1 = br[pos] + func(pos + 1, baki - ar[pos]);
    p2 = func(pos + 1, baki);
    if(p1>p2)
    {
        dp[pos][baki] = p1;
...
void printpath(ll pos, ll baki)
{
    if(dir[pos][baki] == -1) return;
    if(dir[pos][baki] == 1)
    {
        vc.pb(mp(cr[pos],br[pos]));
        printpath(pos + 1, baki - ar[pos]);
    }
    else
    {
        printpath(pos + 1, baki);
    }
}
