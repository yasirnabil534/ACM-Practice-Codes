...
int func(ll n, ll p, int state)
{
    for(ll i = 2; i < 10; i++)
    {
        if(p*i >= n) return state;
        if(M[p*i]==1) return state;
        int x = func(n, p*i, reversestate(state));
        if(x==state)
            M[p*i] = 1;
        else
            M[p*i] = 2;
...
int main()
{
    ...
    while(scln(n)!=EOF)
    {
        int x = func(n, 1LL, 2LL);
        if(x==1) pf("Ollie wins.\n");
        else if(x==2) pf("Stan wins.\n");
        M.clear();
    }
    return 0;
}
