...
int main()
{
    ...
    while(scin(xx)!=EOF)
    {
        ...
        while(!Q.empty())
        {
            ...
            {
                int y = graph[x][i];
                if(active[y]==0&&state[y]<3)
                {
                    state[y]++;
                    if(state[y]==3){
                        ///cout<<"-------"<<y<<' '<<state[y]<<endl;
                        Q.push(y);
                        year[y] = max(year[x] + 1,year[y]);
                        wake.insert(y);
                    }
                }
            }
        }
        sort(year, year + 27, cmp);
        ///cout<<wake.size()<<endl;
        if(wake.size()<xx) pf("THIS BRAIN NEVER WAKES UP\n");
        else pf("WAKE UP IN, %d, YEARS\n",year[0]);
