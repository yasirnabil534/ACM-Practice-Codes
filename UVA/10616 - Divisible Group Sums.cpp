...
int ar[2005],n,q,m,d;
ll dp[205][25][25]; ///pos, sum, nilam, m

ll kp(int pos, int nilam, int sum)
{
    if(nilam==0)
    {
        if(sum%d==0){
    ...
    if(pos==n) return 0;
    if(dp[pos][sum][nilam] != -1){ return dp[pos][sum][nilam];}
    ll p1 = kp(pos + 1, nilam - 1, ((sum + ar[pos]%d)%d + d)%d);
    ll p2 = kp(pos + 1, nilam, sum);
    dp[pos][sum][nilam] = p1 + p2;
    return dp[pos][sum][nilam];
}
